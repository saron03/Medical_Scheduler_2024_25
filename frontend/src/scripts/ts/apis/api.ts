...
export interface Patient {
  ...
  // Login User
  export const loginUser = async (loginData: { email: string; password: string; role: string }): Promise<{ token: string }> => {
    try {
  ...
  // Signup User
  export const signupUser = async (userData: { email: string; username: string; password: string; roleId: number }): Promise<{ token: string }> => {
    try {
      const response = await fetch(
