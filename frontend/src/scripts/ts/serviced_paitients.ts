...
function renderCompletedUsers(): void {
  const completedUsers: CompletedUser[] = JSON.parse(
    localStorage.getItem("completedUsers") || "[]"
  );
  const completedUserTableBody = document.getElementById(
    "completedUserTableBody"
  ) as HTMLElement;
