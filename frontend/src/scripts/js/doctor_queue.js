    ...
    return __generator(this, function (_a) {
      switch (_a.label) {
        ...
        case 2:
          ...
          resolvedPending = users.filter(function (user) {
...
function renderUsers(users) {
  ...
  users.forEach(function (user, index) {
    ...
    var row = '<tr class="'
      .concat(rowClass, '">\n            <td>')
      .concat(user.patient.first_name, " ")
      .concat(user.patient.last_name, "</td>\n            <td>")
      .concat(user.patient.patient_id, "</td>\n            <td>")
      .concat(user.patient.date_of_birth, "</td>\n            <td>")
      .concat(
        user.status === 2
          ? "Pending"
          : user.status === 3
          ? "Resolved Pending"
          : "Not Pending",
        '</td>\n            <td>\n                <button class="btn btn-secondary btn-sm pend-btn" onclick="pendUser('
      )
      .concat(index, ')">\n                    ')
      .concat(
        user.status === 2 ? "Unpend" : user.status === 3 ? "Pend" : "Pend",
        '\n                </button>\n                <button class="btn btn-success btn-sm" onclick="completeUser('
      )
      .concat(index, ')">Complete</button>\n            </td>\n        </tr>');
...
var completeUser = function (index) {
  return __awaiter(_this, void 0, void 0, function () {
    var user, error_2;
    return __generator(this, function (_a) {
      switch (_a.label) {
    ...
    return __generator(this, function (_a) {
      switch (_a.label) {
        ...
        case 1:
          ...
          return [
            4 /*yield*/,
            fetch("http://localhost:4000/api/v1/queues/".concat(user.queue_id)),
          ];
        case 2:
          response = _a.sent();
          if (!response.ok) {
            throw new Error(
              "Failed to fetch user details: ".concat(response.statusText)
            );
          }
          return [4 /*yield*/, response.json()];
        case 3:
          ...
          return [
            4 /*yield*/,
            fetch(
              "http://localhost:4000/api/v1/queues/".concat(
        ...
        case 4:
          ...
          if (!putResponse.ok) {
            throw new Error(
              "Failed to update user status: ".concat(putResponse.statusText)
            );
          }
